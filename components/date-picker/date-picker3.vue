						...
						<view v-for="(monthData, index) in date" :key="index" class="month">
							...
							<view class="month-content">
								<view v-for="(data, index2) in monthData" :key="index2" class="day" :data-index="index" :data-indexs="index2"
								 :class="
										data.re < today
											? (data.date == choiceDate[0].date ? ' begin' : data.date == choiceDate[1].date ? ' end' : '')
											: data.selected
											? (data.date == choiceDate[0].date ? ' begin' : data.date == choiceDate[1].date ? ' end' : '')
											: ''
									"
								 :style="index2 == 0 ? 'margin-left:' + weeks[index] * (100 / 7) + '%' : ''" @tap="dayClick">
									<view class="day-content">
										...
										<text class="day-tip"></text>
									</view>
									<view class="beginTip" v-if="false">请选择离店日期</view>
									<view class="endTip" v-if="false">{{ dayCount2 }}</view>
								</view>
							</view>
						</view>
					</scroll-view>
		...
		methods: {
			...
			dateData: function() {
				...
				if (startDate) {
					...
					if (startDate.getTime() < getDateTime) startDate = new Date(new Date().getTime() - 24 * 3600 * 1000);//new Date();
				} else {
					startDate = new Date(new Date().getTime() - 24 * 3600 * 1000);//new Date();
				}

				if (endDate) {
					endDate = new Date(this.endDate.split('-').join('/'));
					if (endDate.getTime() < getDateTime) endDate = new Date();//new Date(startDate.getTime() + 24 * 3600 * 1000);
				} else {
					endDate = new Date();//new Date(startDate.getTime() + 24 * 3600 * 1000);
				}
				...
				let yearList = [year, year - 1]; //年份最大可能
			...
			selectday: function(index, indexs, isUserClick) {
				...
				if (this.dateFlag && this.dateFlag.date) {
					...
					//console.log(this.choiceDateArr[this.choiceDateArr.length - 1])
