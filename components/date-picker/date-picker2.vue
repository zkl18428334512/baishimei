	...
	<view>
		<view class="order-time" @tap="showCalendar()" v-if="!modal">
			...
			<view class="time-viewer" v-else>
				<text class="goInHotel" style="margin-left: -100rpx;">开始日期</text>
				...
				<text class="left-hotel">截止日期</text>
	...
	export default {
		...
		methods: {
			...
			dateData: function() {
				...
				let yearList = [year-1,year]; //年份最大可能
				...
				for (let i = 0; i < yearList.length; i++) {
					...
					for (let j = 0; j < mList.length; j++) {
						...
						let	t_days1 = [31, 28 + leapYear(yearList[i]), 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
				...
				let start_month = startDate.getMonth() + 12;
				let start_day = startDate.getDate();
				//
				let end_year = endDate.getFullYear();
				let end_month = endDate.getMonth() + 12;
